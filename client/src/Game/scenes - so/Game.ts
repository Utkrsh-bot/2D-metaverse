...
export default class Game extends Phaser.Scene {
	...
	create() {
		...
		const obj2 = map.addTilesetImage(
			"Modern_Office_Black_Shadow",
			"tiles5"
		);
		const obj3 = map.addTilesetImage("Classroom_and_library", "tiles6");

		const ground = [ground1, ground2, ground3, obj1].filter(
			(obj): obj is Phaser.Tilemaps.Tileset => obj !== null
		);
		const office_obj = [obj1, obj2, obj3].filter(
			(obj): obj is Phaser.Tilemaps.Tileset => obj !== null
		);
